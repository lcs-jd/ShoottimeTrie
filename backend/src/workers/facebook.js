...
// Pause entre chaque photo pour éviter le blocage anti-spam Meta
...
async function uploadPhotoToAlbum(albumId, photoPath, caption) {
  ...
  const res = await fetch(`${FB_API_BASE}/${albumId}/photos`, {
    method: 'POST',
    body: form,
  });
...
export const facebookWorker = new Worker('facebook', async (job) => {
  ...
  if (!PAGE_ID || !ACCESS_TOKEN) {
    throw new Error('FACEBOOK_PAGE_ID ou FACEBOOK_PAGE_ACCESS_TOKEN non configurés');
  }
  ...
  connection,
  concurrency: 1, // séquentiel pour respecter le rate-limit Meta
});

facebookWorker.on('failed', (job, err) => {
  console.error(`[facebook] job ${job?.id} failed:`, err.message);
  if (job?.data?.photoId) {
    db.prepare("UPDATE photos SET status = 'facebook_error' WHERE id = ?").run(job.data.photoId);
    broadcast(job.data.sessionId, { type: 'facebook_error', photoId: job.data.photoId, error: err.message });
  }
});
